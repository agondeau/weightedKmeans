...
int main(int argc, char *argv[]) {
    ...
    if(argc > 1)
    {
        ...
        uint32_t nbRep = 1, kmax = 2, kmin = 2;
        ...
        for(i=1;i<argc;i++)
        {
            if(!strcmp("-i", argv[i]))
            {
                if(argv[i+1] != NULL && strcmp("-r", argv[i+1]) && strcmp("-kmin", argv[i+1]) && strcmp("-kmax", argv[i+1]) && strcmp("-o", argv[i+1]) && strcmp("-f", argv[i+1]) && strcmp("-h", argv[i+1]))
                {
            ...
            {
                if(argv[i+1] != NULL && strcmp("-i", argv[i+1]) && strcmp("-kmin", argv[i+1]) && strcmp("-kmax", argv[i+1]) && strcmp("-o", argv[i+1]) && strcmp("-f", argv[i+1]) && strcmp("-h", argv[i+1]))
                {
                    nbRep = atoi(argv[i+1]);
                    i++;
                }
            ...
            else if(!strcmp("-kmin", argv[i]))
            {
                if(argv[i+1] != NULL && strcmp("-i", argv[i+1]) && strcmp("-r", argv[i+1]) && strcmp("-o", argv[i+1]) && strcmp("-f", argv[i+1]) && strcmp("-h", argv[i+1]) && strcmp("-kmax", argv[i+1]))
                {
                    kmin = atoi(argv[i+1]);
                    i++;
                }
            ...
            else if(!strcmp("-kmax", argv[i]))
            {
                if(argv[i+1] != NULL && strcmp("-i", argv[i+1]) && strcmp("-r", argv[i+1]) && strcmp("-o", argv[i+1]) && strcmp("-f", argv[i+1]) && strcmp("-h", argv[i+1]) && strcmp("-kmin", argv[i+1]))
                {
                    kmax = atoi(argv[i+1]);
                    i++;
                }
            ...
            else if(!strcmp("-o", argv[i]))
            {
                ...
                if(argv[i+1] != NULL && strcmp("-i", argv[i+1]) && strcmp("-r", argv[i+1]) && strcmp("-kmin", argv[i+1]) && strcmp("-kmax", argv[i+1]) && strcmp("-f", argv[i+1]) && strcmp("-h", argv[i+1]))
                {
            ...
            else if(!strcmp("-f", argv[i]))
            {
                ...
                if(argv[i+1] != NULL && strcmp("-i", argv[i+1]) && strcmp("-r", argv[i+1]) && strcmp("-kmin", argv[i+1]) && strcmp("-kmax", argv[i+1]) && strcmp("-o", argv[i+1]) && strcmp("-h", argv[i+1]))
                {
        ...
        INF("Computed from %d to %d clusters with %d random starts", kmin, kmax, nbRep);
        ...
        if(dat != NULL)
        {
            ...
            if(featuresWeightedKmeans && !objectsWeightedKmeans)
            {
                ...
                CLUSTER_computeFeaturesWeightedKmeans(dat, n, p, kmin, kmax, nbRep, internalFeatureWeights, featureWeightFileName, feaWeiMet);
            }
            ...
            else if((!featuresWeightedKmeans && objectsWeightedKmeans) || (featuresWeightedKmeans && objectsWeightedKmeans))
            {
                ...
                CLUSTER_computeWeightedKmeans(dat, n, p, kmin, kmax, nbRep, internalFeatureWeights, featureWeightFileName, feaWeiMet, internalObjectWeights, objectWeightFileName, objWeiMet);
            }
            // Classical k-means
            else
            {
                INF("Classic k-means");
                CLUSTER_computeKmeans(dat, n, p, kmin, kmax, nbRep);
            }
...
void usage(void)
{
    WRN("Usage : kmeans -i inputDataFile [-r nbReplicates] [-kmin clusterMin] [-kmax clusterMax] [-o objectsWeightsFile/objectsWeightsMethod] [-f featuresWeightsFile/featuresWeightsMethod]");
    WRN("");
    WRN("-i : Specify the input file containing the header (number of data + tab + number of variables) and the data tab spaced.");
    WRN("-r : Specify the number of random starts (default 1).");
    WRN("-kmin : Specify the number min of clusters. K-means will be computed for clusterMin to clusterMax groups (default 2).");
    WRN("-kmax : Specify the number max of clusters. K-means will be computed for clusterMin to clusterMax groups (default 2).");
